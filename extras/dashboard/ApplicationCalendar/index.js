		...
		return {
            ...
            controller: function($scope) {
                ...
                $q.all([
                    ...
                    Restangular.all('application').getList({ createdAt: [ now.clone().add('days', -1).format(), now.clone().format() ].join(',') }),
                    Restangular.all('application').getList({ createdAt: [ now.clone().add('days', -7).format(), null ].join(',') }),
                    Restangular.all('application').getList({ createdAt: [ now.clone().add('days', -30).format(), null ].join(',') })
                ])
